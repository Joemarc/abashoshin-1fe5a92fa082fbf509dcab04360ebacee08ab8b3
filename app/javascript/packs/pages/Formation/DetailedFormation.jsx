...
class DetailedFormation extends Component {
  componentDidMount() {
    ...
    validateTokenAction().then(this.getFormationData()).catch(this.getFormationData);
  }
  ...
  render() {
    ...
    const renderFormation = () => {
      ...
      } else if (formation) component = <FormationView formation={formation} />;
    ...
    const renderModules = () => {
      let component;
      if (isLoadingModules) {
        component = [1, 2, 3, 4].map( m => <LoaderCardArticle key={m.idmodule}/>);
      } else if (modules.length) component = modules.map(m => <ModulesView module={m} />);
      else component = 'Aucune formations à afficher pour le moment';
      return component;
    };
