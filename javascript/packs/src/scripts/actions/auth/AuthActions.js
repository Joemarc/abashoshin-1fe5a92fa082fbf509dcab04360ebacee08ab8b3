    ...
    return API.post('/auth/sign_in', {...user})
      .then(response => dispatch(signInSuccess(response),
           ...
           localStorage.setItem('authHeaders', JSON.stringify(
             ...
             'client': response.headers["client"], 'expiry': response.headers["expiry"],
             'token-type': response.headers["token-type"], 'uid': response.headers["uid"]
             }))
      ))
...
export function validateToken() {
  return dispatch => {
    ...
    return API.get('/auth/validate_token')
      ...
      error => Promise.reject(dispatch(validateTokenFailure(error)))))
  };
}
